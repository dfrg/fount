...
impl<'a> Zone<'a> {
    ...
    fn point_mut<'z>(&'z mut self, index: usize) -> Option<&'z mut Point> {
        self.points.get_mut(index)
    }

    fn original(&self, index: usize) -> Option<Point> {
        self.original.get(index).copied()
    }

    fn original_mut<'z>(&'z mut self, index: usize) -> Option<&'z mut Point> {
        self.original.get_mut(index)
    }

    fn unscaled(&self, index: usize) -> Option<Point> {
        self.unscaled.get(index).copied()
    }

    fn unscaled_mut<'z>(&'z mut self, index: usize) -> Option<&'z mut Point> {
        self.unscaled.get_mut(index)
    }

    fn flags(&self, index: usize) -> Option<PointFlags> {
        self.flags.get(index).copied()
    }

    fn flags_mut<'z>(&'z mut self, index: usize) -> Option<&'z mut PointFlags> {
        self.flags.get_mut(index)
    }

    fn contour(&self, index: usize) -> Option<u16> {
        self.contours.get(index).copied()
    }

    // fn point_mut<'b>(&'b mut self, index: usize) -> Result<&'b mut Point, HintError> {
    //     self.points
    //         .get_mut(index)
    //         .ok_or(HintError::InvalidPointReference)
    // }

    // fn flags_mut<'b>(&'b mut self, index: usize) -> Result<&'b mut PointFlags, HintError> {
    //     self.flags
    //         .get_mut(index)
    //         .ok_or(HintError::InvalidPointReference)
    // }
    ...
    fn shift(&mut self, is_x: bool, p1: usize, p2: usize, p: usize) -> Option<()> {
        ...
        macro_rules! shift_coord {
            ($coord:ident) => {
                ...
                if delta != 0 {
                    let (first, second) = self.points.get_mut(p1..=p2)?.split_at_mut(p - p1);
        ...
        macro_rules! interpolate_coord {
            ($coord:ident) => {
                ...
                let iter = self
                    ...
                    .get(p1..=p2)?
                    .iter()
                    .zip(self.unscaled.get(p1..=p2)?)
                    .zip(self.points.get_mut(p1..=p2)?);
...
pub struct Interpreter<'a> {
    store: &'a mut [i32],
    cvt: &'a mut [i32],
    fdefs: &'a mut [Definition],
    idefs: &'a mut [Definition],
    twilight: Zone<'a>,
    glyph: Zone<'a>,
...
impl<'a> Interpreter<'a> {
    ...
    pub fn new(
        storage: &'a mut [i32],
        cvt: &'a mut [i32],
        function_defs: &'a mut [Definition],
        instruction_defs: &'a mut [Definition],
        twilight: Zone<'a>,
        glyph: Zone<'a>,
        coords: &'a [NormalizedCoord],
        axis_count: u16,
...
impl<'a> Interpreter<'a> {
    ...
    #[inline(always)]
    fn zone_mut<'b>(&'b mut self, zone: u8) -> &'b mut Zone<'a> {
        if zone == 1 {
    ...
    fn move_point(&mut self, zone: u8, point_ix: usize, distance: i32) -> Option<()> {
        let legacy = self.v35;
        let bc = self.compat;
        let iupx = self.iupx;
        let iupy = self.iupy;
        ...
        loop {
            ...
            match opcode {
                ...
                op::SPVTL0..=op::SFVTL1 => {
                    ...
                    let p1 = self.zp1().point(index2)?;
                    let p2 = self.zp2().point(index1)?;
                    let mut a = p1.x - p2.x;
                    let mut b = p1.y - p2.y;
                    let mut op = opcode;
                    if a == 0 && b == 0 {
                ...
                op::ISECT => {
                    ...
                    if 19 * discriminant.abs() > dp.abs() {
                        ...
                        let point = self.zp2_mut().point_mut(point_ix)?;
                        point.x = pa0.x + x;
                        point.y = pa0.y + y;
                    } else {
                        let point = self.zp2_mut().point_mut(point_ix)?;
                ...
                op::ALIGNPTS => {
                    ...
                    let distance = self.project(self.zp0().point(p2)?, self.zp1().point(p1)?) / 2;
                ...
                op::UTP => {
                    ...
                    self.zp0_mut().flags_mut(point_ix)?.clear_marker(marker)
                }
                op::LOOPCALL | op::CALL => {
                ...
                op::MDAP0 | op::MDAP1 => {
                    ...
                    let mut distance = 0;
                    if (opcode & 1) != 0 {
                        let c = self.fast_project(self.zp0().point(point)?);
                ...
                op::IUP0 | op::IUP1 => {
                    ...
                    if run {
                        ...
                        for i in 0..self.glyph.contours.len() {
                            let mut end_point = self.glyph.contour(i)? as usize;
                            let first_point = point;
                            if end_point >= self.glyph.points.len() {
                                end_point = self.glyph.points.len() - 1;
                            }
                            while point <= end_point && !self.glyph.flags(point)?.has_marker(marker)
                            {
                                point += 1;
                            }
                            if point <= end_point {
                                ...
                                while point <= end_point {
                                    if self.glyph.flags(point)?.has_marker(marker) {
                ...
                op::IP => {
                    ...
                    let orus_base = if in_twilight {
                        self.zp0().original(rp1)?
                    } else {
                        self.zp0().unscaled(rp1)?
                    };
                    let cur_base = self.zp0().point(rp1)?;
                    let old_range = if in_twilight {
                        self.dual_project(self.zp1().original(rp2)?, orus_base)
                    } else {
                        self.dual_project(self.zp1().unscaled(rp2)?, orus_base)
                    };
                    let cur_range = self.project(self.zp1().point(rp2)?, cur_base);
                    while loop_counter > 0 {
                        loop_counter -= 1;
                        args_top -= 1;
                        let point = stack.get(args_top)? as usize;
                        let original_distance = if in_twilight {
                            self.dual_project(self.zp2().original(point)?, orus_base)
                        } else {
                            self.dual_project(self.zp2().unscaled(point)?, orus_base)
                        };
                        let cur_distance = self.project(self.zp2().point(point)?, cur_base);
                ...
                op::MSIRP0 | op::MSIRP1 => {
                    ...
                    if self.zp1 == 0 {
                        *self.zp1_mut().point_mut(point)? = self.zp0().original(rp0)?;
                        self.move_original(self.zp1, point, stack.get(args + 1)?)?;
                        *self.zp1_mut().point_mut(point)? = self.zp1().original(point)?;
                    }
                    let d = self.project(self.zp1().point(point)?, self.zp0().point(rp0)?);
                ...
                op::ALIGNRP => {
                    ...
                    while loop_counter > 0 {
                        ...
                        let point = stack.get(args_top)? as usize;
                        let distance =
                            self.project(self.zp1().point(point)?, self.zp0().point(rp0)?);
                ...
                op::MIAP0 | op::MIAP1 => {
                    ...
                    let original_distance = self.fast_project(self.zp0().point(point_ix)?);
                    if (opcode & 1) != 0 {
                    ...
                    self.move_point(self.zp0, point_ix, distance - original_distance)?;
                    rp0 = point_ix;
                    rp1 = point_ix;
                }
                op::WS => {
                    let index = stack.get(a0)? as usize;
                    *self.store.get_mut(index)? = stack.get(a1)?;
                }
                op::RS => {
                ...
                op::WCVTP => {
                    let index = stack.get(a0)? as usize;
                    *self.cvt.get_mut(index)? = stack.get(a1)?;
                }
                op::WCVTF => {
                    let index = stack.get(a0)? as usize;
                    *self.cvt.get_mut(index)? = mul(stack.get(a1)?, self.scale);
                }
                op::RCVT => {
                    let sp = stack.get_mut(a0)?;
                    let index = *sp as usize;
                ...
                op::GC0 | op::GC1 => {
                    let index = stack.get(a0)? as usize;
                    let r = if (opcode & 1) != 0 {
                        self.fast_dual_project(self.zp2().original(index)?)
                    } else {
                        self.fast_project(self.zp2().point(index)?)
                    };
                    *stack.get_mut(a0)? = r;
                }
                op::SCFS => {
                    let index = stack.get(a0)? as usize;
                    let a = self.fast_project(self.zp2().point(index)?);
                    self.move_point(self.zp2, index, stack.get(a1)?.wrapping_sub(a))?;
                    if self.zp2 == 0 {
                        *self.twilight.original_mut(index)? = self.twilight.point(index)?;
                    }
                }
                op::MD0 | op::MD1 => {
                    ...
                    let d = if (opcode & 1) != 0 {
                        self.project(self.zp0().point(b)?, self.zp1().point(a)?)
                    } else if self.zp0 == 0 || self.zp1 == 0 {
                        self.dual_project(self.zp0().original(b)?, self.zp1().original(a)?)
                    } else {
                        mul(
                            self.dual_project(self.zp0().unscaled(b)?, self.zp1().unscaled(a)?),
                            self.yscale,
                        )
                    };
                    *stack.get_mut(a0)? = d;
                ...
                op::FLIPPT => {
                    if !self.v35 && self.compat && self.iupx && self.iupy {
                        ...
                        while loop_counter > 0 {
                            args_top -= 1;
                            let point = stack.get(args_top)? as usize;
                            self.glyph.flags.get_mut(point)?.flip_on_curve();
                            loop_counter -= 1;
                        }
                    }
                    loop_counter = 1;
                ...
                op::FLIPRGON | op::FLIPRGOFF => {
                    if !self.v35 && self.compat && self.iupx && self.iupy {
                        ...
                        if opcode == op::FLIPRGON {
                            for tag in self.glyph.flags.get_mut(b..=a)? {
                                tag.set_on_curve();
                            }
                        } else {
                            for tag in self.glyph.flags.get_mut(b..=a)? {
                ...
                op::SDPVTL0 | op::SDPVTL1 => {
                    ...
                    let mut b;
                    {
                        let v1 = self.zp1().original(p2)?;
                        let v2 = self.zp2().original(p1)?;
                        a = v1.x - v2.x;
                        b = v1.y - v2.y;
                        if a == 0 && b == 0 {
                            a = 0x4000;
                    ...
                    self.project.dv = v;
                    {
                        let v1 = self.zp1().point(p2)?;
                        let v2 = self.zp2().point(p1)?;
                        a = v1.x - v2.x;
                        b = v1.y - v2.y;
                        if a == 0 && b == 0 {
                            a = 0x4000;
                ...
                op::MDRP00000..=op::MDRP11111 => {
                    ...
                    if self.zp0 == 0 || self.zp1 == 0 {
                        original_distance = self.dual_project(
                            self.zp1().original(point_ix)?,
                            self.zp0().original(rp0)?,
                        );
                    } else {
                        let v1 = self.zp1().unscaled(point_ix)?;
                        let v2 = self.zp0().unscaled(rp0)?;
                    ...
                    original_distance =
                        self.project(self.zp1().point(point_ix)?, self.zp0().point(rp0)?);
                    self.move_point(self.zp1, point_ix, distance.wrapping_sub(original_distance))?;
                    rp1 = rp0;
                    rp2 = point_ix;
                    if (opcode & 16) != 0 {
                        rp0 = point_ix;
                ...
                op::MIRP00000..=op::MIRP11111 => {
                    ...
                    if self.zp1 == 0 {
                        let fv = self.project.fv;
                        let p = {
                            let p2 = self.zp0().original(rp0)?;
                            let p1 = self.zp1_mut().original_mut(point)?;
                            p1.x = p2.x + mul(cvt_distance, fv.x as i32);
                            p1.y = p2.y + mul(cvt_distance, fv.y as i32);
                            *p1
                        };
                        *self.zp1_mut().point_mut(point)? = p;
                    }
                    let original_distance =
                        self.dual_project(self.zp1().original(point)?, self.zp0().original(rp0)?);
