...
impl FontContext {
    ...
    /// Returns the font family for the specified identifier.
    ...
    pub fn family_by_name<'a>(&'a self, name: &str) -> Option<FontFamily> {
        self.sync_user();
    ...
    /// Returns the font for the specified identifier.
    ...
    /// Returns the font source for the specified identifier.
    ...
    pub fn register_fonts(&self, data: Vec<u8>) -> Option<Registration> {
        ...
        let count = collection.add_fonts(&mut scanner, Arc::new(data), Some(&mut reg)).unwrap_or(0);
        if count != 0 {
            self.library.inner.user_version.fetch_add(1, Ordering::Relaxed);
            Some(reg)
        } else {
            None
        }
